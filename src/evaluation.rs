...
const MG_TABLE: [[i32; 64]; 6] = [PAWN_MG, KNIGHT_MG, BISHOP_MG, ROOK_MG, QUEEN_MG, KING_MG];
const EG_TABLE: [[i32; 64]; 6] = [PAWN_EG, KNIGHT_EG, BISHOP_EG, ROOK_EG, QUEEN_EG, KING_EG];

fn piece_value(typ: PieceType) -> i32 {
    match typ {
        PieceType::Pawn => 100,
...
pub fn evaluate(board: &Board) -> i32 {
    ...
    for i in 0..64 {
        if let Some(piece) = board.piece_at(i) {
            ...
            let square_index = pst_index(i, color);
            let piece_index = match typ {
                PieceType::Pawn => 0,
                PieceType::Knight => 1,
                PieceType::Bishop => 2,
                PieceType::Rook => 3,
            ...
            let mg_bonus = MG_TABLE[piece_index][square_index];

            let eg_bonus = EG_TABLE[piece_index][square_index];

            game_phase += match typ {
                PieceType::Pawn => 0,
                PieceType::Knight => 1,
                PieceType::Bishop => 1,
                PieceType::Rook => 2,
            ...
            match color {
                Color::White => {
                    mg_score += piece_value(typ) + mg_bonus;
                    eg_score += piece_value(typ) + eg_bonus
                }
                Color::Black => {
                    mg_score -= piece_value(typ) + mg_bonus;
                    eg_score -= piece_value(typ) + eg_bonus
    ...
    (mg_score * game_phase + eg_score * (24 - game_phase)) / 24 // linear interpolation blend
}

fn pst_index(square: usize, color: Color) -> usize {
    match color {
        Color::White => 56 - (square / 8) * 8 + square % 8, // flip board vertically
        Color::Black => square,
    }
}
